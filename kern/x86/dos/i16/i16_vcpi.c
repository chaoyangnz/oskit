...
static oskit_addr_t vcpi_pdir, vcpi_ptable0;
...
{
	extern oskit_addr_t boot_image_pa;
	...
	: "a" ((unsigned short)0xde0c),
	  "S" (boot_image_pa + (oskit_addr_t)&vcpi_switch_data),
	  "i" (KERNEL_DS)
	: "eax", "edx", "esi");
...
void i16_vcpi_check()
{
	extern oskit_addr_t dos_mem_phys_free_mem;
	extern oskit_addr_t dos_mem_phys_free_size;
	...
	if (cpu[0].tables.gdt[VCPI_CS_IDX].granularity & SZ_G)
		i16_assert(vcpi_pmode_entry.ofs <
			   (((oskit_addr_t)cpu[0].tables.gdt[VCPI_CS_IDX].limit_high << 28)
			    | ((oskit_addr_t)cpu[0].tables.gdt[VCPI_CS_IDX].limit_low << 12)
			    | (oskit_addr_t)0xfff));
	else
		i16_assert(vcpi_pmode_entry.ofs <
			   (((oskit_addr_t)cpu[0].tables.gdt[VCPI_CS_IDX].limit_high << 16)
			    | (oskit_addr_t)cpu[0].tables.gdt[VCPI_CS_IDX].limit_low));
	...
	vcpi_switch_data.lin_gdt = boot_image_pa+(oskit_addr_t)&gdt_pdesc.limit;
	vcpi_switch_data.lin_idt = boot_image_pa+(oskit_addr_t)&idt_pdesc.limit;
	vcpi_switch_data.tss_sel = KERNEL_TSS;
	vcpi_switch_data.entry_eip = (unsigned short)(oskit_addr_t)&pmode_return;
	...
	gdt_pdesc.linear_base = boot_image_pa + (oskit_addr_t)&cpu[0].tables.gdt;
	idt_pdesc.limit = sizeof(cpu[0].tables.idt)-1;
	idt_pdesc.linear_base = boot_image_pa + (oskit_addr_t)&cpu[0].tables.idt;
	...
	i16_fill_gdt_descriptor(&cpu[0], KERNEL_TSS,
				boot_image_pa + (oskit_addr_t)&cpu[0].tables.tss,
	...
	i16_do_32bit(
		...
		VCPI_PAGING_INIT(vcpi_pdir, (oskit_addr_t)first_free_pte / 4 * PAGE_SIZE);
