...
#else /* The OSKit cannot support yielding or delay */
                    int timer;
                    unsigned long flags;

                    timer = jiffies + time1;

                    /* Signal to user that the OSKit in itself is
                     * incomplete. For GNUmach a usermode driver,
                     * or at least the initilization can be done
                     * in user space rather than in the kernel.
                     */
                    printk ("OSKit: linux/src/drivers/scsi/sd.c need sleep() function.\n");

                    save_flags (flags);

                    /* We allow the kernel to interrupt us while we
                     * busy loop waiting for one second to pass.
                     */
                    sti ();
                    while (timer < jiffies)
                      ;
