...
int	oskit_linux_ofw_probe_ethernet(int *ioa, int *irq, unsigned char *mac);
int	oskit_force_fullduplex = 1;
#endif
...
#ifdef OSKIT_ARM32_SHARK
	if (oskit_linux_ofw_probe_ethernet(&ioaddr, &dev->irq, dev->dev_addr))
		goto err;
#else
...
#ifdef OSKIT_ARM32
	skb = alloc_skb(length + sizeof(short), GFP_ATOMIC);
#else
...
#ifdef OSKIT_ARM32
	/*
	 * ACK! Need to align the ip header since the ether header is
	 * only 14 bytes. But, the oskit bufio model is sucky for this,
	 * since higher level code depends on being able to map the data
	 * buffer directly to avoid copies, and assumes it just a contiguous
	 * buffer. But, the IP code in the freebsd code assumes that the
	 * ip header it gets in the mbuf is properly word aligned for word
	 * access. The only way to arrange that would be for the glue code
	 * to copy data. So, just push the ether header up 2 bytes, thus
	 * making the ip header start on a word boundry. Thats okay since the
	 * ether header does not have any word sized fields in it and doing
	 * it this way means no one else has to worry about it, although
	 * other drivers would need to do the same thing.
	 */
	skb_reserve(skb, sizeof(short));
#endif
