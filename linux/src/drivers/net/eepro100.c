...
enum speedo_offsets {
	...
	SCBIntrCtrl = 3,			/* Interrupt control. */
...
#if defined(OSKIT) && defined(DEVICE_POLLING)
	int polling;		/* type of polling in effect, values defined below */
	int rx_stalled;
	long polling_rxints;
	long polling_txints;
#endif
...
#define SCBMaskRxDone			0x4000
#define SCBMaskCmdIdle			0x2000

static int
speedo_rx_poll(struct device *dev, int count, struct sk_buff *skb_array[]);
static void
speedo_setup_polling(struct device *dev, int rx, int tx);

/*
 * XXX define polling bits same as appropriate interrupt mask bits
 * to simplify definition of OUTSCBCMD.
 */
#define POLLING_RX	SCBMaskRxDone
#define POLLING_TX	SCBMaskCmdIdle

/* XXX note implicit sp parameter */
#define OUTSCBCMD(x, ioaddr)	outw(sp->polling|(x), (ioaddr) + SCBCmd)
#else
#define OUTSCBCMD(x, ioaddr)	outw((x), (ioaddr) + SCBCmd)
#endif
										...
										int chip_idx, int card_idx)
{
	...
	*/
	...
	dev->setup_polling = speedo_setup_polling;
#endif
...
#if defined(OSKIT) && defined(DEVICE_POLLING)
		/* Should never happen when polling! */
		if (sp->polling & POLLING_TX) {
			printk(KERN_DEBUG "%s: device busy in polling mode!\n", dev->name);
			return 1;
		}
#endif
...
#if defined(OSKIT) && defined(DEVICE_POLLING)
	if ((sp->polling & POLLING_TX) && sp->tx_full) {
		if (speedo_tx_reap(dev) == 0) {
			spin_unlock_irqrestore(&sp->lock, flags);
			clear_bit(0, (void*)&dev->tbusy);
			return 1;
		}
...
#if defined(OSKIT) && defined(DEVICE_POLLING)
	/* when polling, always clear tbusy */
	if (sp->polling & POLLING_TX) {
...
speedo_setup_polling(struct device *dev, int rx, int tx)
{
	...
	if (rx) {
		wait_for_cmd_done(ioaddr + SCBCmd);
		outw(0x4000, ioaddr + SCBCmd);
		sp->polling |= POLLING_RX;
	}
	if (tx) {
		sp->polling |= POLLING_TX;
	}
}
...
speedo_rx_poll(struct device *dev, int count, struct sk_buff *skb_array[])
{
	...
	/*
	 * XXX hack: poll on TX device forces flush/reap of all TX requests
	 */
	if (skb_array == 0 && (sp->polling & POLLING_TX)) {
		while (sp->cur_tx - sp->dirty_tx > 0) {
			if (speedo_debug > 5)
				printk(KERN_DEBUG "%s purging tx queue, cur=%d dirty=%d\n",
	...
	/*
	 * Look for stalled RX, which was caught in the interrupt handler.
	 */
	if (sp->rx_stalled &&
		!(sp->rx_ringp[sp->cur_rx % RX_RING_SIZE]->status & RxComplete)) {
		long ioaddr = dev->base_addr;
