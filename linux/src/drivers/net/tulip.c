...
static int
tulip_rx_poll(struct device *dev, int count, struct sk_buff *skb_array[]);
static void
tulip_setup_polling(struct device *dev, int rx, int tx);
#endif

								   ...
								   int chip_idx, int board_idx)
{
	...
	dev->setup_polling = tulip_setup_polling;
#endif
...
tulip_setup_polling(struct device *dev, int rx, int tx)
{
	...
	if (rx) {
		csr7 = inl(ioaddr + CSR7) & ~(RxNoBuf|RxIntr);
