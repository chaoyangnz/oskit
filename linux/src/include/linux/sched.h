...
#ifdef OSKIT
/*
 * MAKE SURE THE COMMON FIELDS ARE FIRST IN THIS STRUCTURE,
 * that way it can be used safely in code compiled with or without OSKIT_FS.
 */
#include <oskit/dev/osenv_sleep.h>
/*
 * Even though devices in the fdev framework
 * don't really have any idea of what "processes" are in the base OS,
 * we can still simulate the notion of tasks
 * to the extent needed by the Linux device drivers.
 * At every process-level entrypoint into the Linux driver set,
 * we do an OSKIT_LINUX_CREATE_CURRENT,
 * which creates a task_struct on the local stack and points 'current' to it;
 * then around every blocking call back out to the OS
 * we must save and resture this 'current' pointer,
 * so that 'current' effectively always points to a per-process structure.
 * See sched.h (not include/linux/sched.h) for definitions of these macros.
 */
struct task_struct {
	/*
	 * Used by fsuser() macro; always zero for OSKIT_DEV but holds
	 * legitimate values for OSKIT_FS.
	 */
	int fsuid;

	/*
	 * This sleeprec is used as the rendezvous
	 * between a blocked process-level activity in the Linux environment
	 * and an interrupt handler that wants to wake up a specific process.
	 */
	osenv_sleeprec_t sleeprec;

	/*
	 * Current address space descriptor.  See uaccess.h.
	 * Unused right now, but may want it someday.
	 */
	mm_segment_t addr_limit;
#ifdef OSKIT_FS
	volatile long state;	/* -1 unrunnable, 0 runnable, >0 stopped */
	long need_resched; /* Used by random.c */
	long counter;
	int groups[NGROUPS];
	unsigned short uid /* fsuid below */;
	unsigned short gid, fsgid;
...
#ifdef OSKIT_FS
extern int fs_linux_current_time(void);
#define CURRENT_TIME (fs_linux_current_time())
#else
...
#ifdef OSKIT_FS
/* This is referenced for pipes but should never be called. */
...
#if defined(OSKIT_DEV)
#define suser()		1
#define fuser()		1
#define capable(cap)	1
#elif defined(OSKIT_FS)
/* We don't have any effective-uid notion so just check fsuid like fsuser(). */
#define suser()		(current->fsuid == 0)
#define fuser()		(current->fsuid == 0)
#define fsuser()	(current->fsuid == 0)
#define capable(cap)	(current->fsuid == 0)
#else /* not OSKIT_DEV nor OSKIT_FS */
/*
