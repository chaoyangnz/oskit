...
#ifndef OSKIT_DEV
/* OSKIT_DEV doesn't need fs stuff, might as well save compile time. */
...
#ifdef OSKIT
/* This is to prevent us from having to pull in a bunch of rpc crap. */
...
#ifdef OSKIT_DEV
	/*
	 * Need a place to stash the inode since the device only stuff does
	 * not use the buffer cache, and thats how the inode is now tracked.
...
#ifndef OSKIT_DEV
/* OSKIT_DEV doesn't need superblock stuff, might as well save compile time. */
...
#ifdef OSKIT_FS
/* Make this helper function in super.c external. */
...
/* This refers to the super_block, but we do not need this stuff anyway. */
...
#ifdef OSKIT_DEV
/*
 * This is normally in fs/buffer.c but OSKIT_DEV doesn't need that
 * whole file and plus this enables inlining.
 */
extern inline void
...
{
#ifdef OSKIT_DEV
	/* OSKIT_DEV doesn't have any buffer lists. */
...
#ifdef OSKIT_FS
/* We have no way of knowing this, so we say "no". */
#define check_disk_change(d) 0
#else
extern int check_disk_change(kdev_t dev);
#endif
#ifdef OSKIT_DEV
/* OSKIT_DEV has no buffer cache stuff. */
#define invalidate_inodes(dev)
#define invalidate_inode_pages(inode)
#define invalidate_buffers(dev)
#define destroy_buffers(dev)
#else
...
#ifdef OSKIT_DEV
/* OSKIT_DEV has no uid-type concept. */
#define vfs_permission(i, m) 0
...
#ifdef OSKIT_FS
/* These do_foo functions are normally static but we've undone that
   so we can use them. */
extern int do_mkdir(const char * pathname, int mode);
extern int do_rmdir(const char * name);
extern int do_unlink(const char * name);
extern int do_symlink(const char * oldname, const char * newname);
extern int do_link(struct dentry * dentry, const char * newname);
extern int do_rename(const char * oldname, const char * newname);
#endif /* OSKIT_FS */
...
#ifdef OSKIT_DEV
/* OSKIT_DEV does not need to worry about superblocks */
#define set_blocksize(dev, size)
#else
...
#ifdef OSKIT_FS
/* generic_file_mmap is used in vtables but we make it NULL so it isn't
   ever referenced. */
...
#ifdef OSKIT_DEV
/* OSKIT_DEV does not need to worry about superblocks */
#define get_super(dev) ((struct super_block *) 0)
#else
...
#ifdef OSKIT_FS
/* We use a slightly modified mount_root. */
extern int mount_root(kdev_t, int, struct super_block **);
#else
